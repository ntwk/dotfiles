...
;; 'rxvt' replaced by 'screen'.  If you are used to the colors defined
;; in rxvt.el, this will give you the same colors when running emacs
;; inside tmux.  tmux uses a default terminal type of 'screen' or one
;; of its derivatives (e.g. screen-256color, etc.), hence the need for
;; this file.
...
(defvar screen-function-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; (define-key screen-function-map "\e[23~" [f11])
    ;; (define-key screen-function-map "\e[24~" [f12])
    ...
    map)
  "Function key overrides for screen.")

(defvar screen-alternatives-map
  (let ((map (make-sparse-keymap)))
...
(defun terminal-init-screen ()
  ...
  (let ((map (copy-keymap screen-alternatives-map)))
  ...
  (screen-set-background-mode)
  ;; This recomputes all the default faces given the colors we've just set up.
  (tty-set-up-initial-frame-faces))

;; Set up colors, for those versions of screen that support it.
(defvar screen-standard-colors
  ;; The names of the colors in the comments taken from the screen.1 man
  ...
  "Names of 16 standard screen colors, their numbers, and RGB values.")

(defun screen-rgb-convert-to-16bit (prim)
  "Convert an 8-bit primary color value PRIM to a corresponding 16-bit value."
  (min 65535 (round (* (/ prim 255.0) 65535.0))))

(defun screen-register-default-colors ()
  "Register the default set of colors for screen or compatible emulator.

This function registers the number of colors returned by `display-color-cells'
for the currently selected frame."
  (let* ((ncolors (display-color-cells))
	 (colors screen-standard-colors)
	 (color (car colors)))
    (if (> ncolors 0)
    ...
    (while (and (> ncolors 0) colors)
      (tty-color-define (car color) (cadr color)
			(mapcar 'screen-rgb-convert-to-16bit
				(car (cddr color))))
	...
	(while (> ncolors 0)
	  (setq color (screen-rgb-convert-to-16bit (+ 8 (* (- 24 ncolors) 10))))
	  (tty-color-define (format "color-%d" (- 256 ncolors))
			    (- 256 ncolors)
			    (list color color color))
	  (setq ncolors (1- ncolors))))

       ((= ncolors 72) ; screen-unicode
	;; 64 non-gray colors
	...
	(while (> ncolors 0)
	  (setq color (screen-rgb-convert-to-16bit
		       (floor
			(if (= ncolors 8)
			    46.36363636
	  ...
	  (tty-color-define (format "color-%d" (- 88 ncolors))
			    ...
			    (list color color color))
	  (setq ncolors (1- ncolors))))
       (t (error "Unsupported number of screen colors (%d)" (+ 16 ncolors)))))
    ;; Modifying color mappings means realized faces don't use the
    ;; right colors, so clear them.
    (clear-face-cache)))

;; screen puts the default colors into an environment variable
;; COLORFGBG.  We use this to set the background mode in a more
;; intelligent way than the default guesswork in startup.el.
(defun screen-set-background-mode ()
  "Set background mode as appropriate for the default screen colors."
  (let ((fgbg (getenv "COLORFGBG"))
    ...
    (when (and fgbg
      ...
      ;; The next line assumes that screen-standard-colors are ordered
      ;; by the color index in the ascending order!
      (setq rgb (car (cddr (nth bg screen-standard-colors))))
      ;; See the commentary in frame-set-background-mode about the
      ;; computation below.
      (if (< (apply '+ rgb)
	     ;; The following line assumes that white is the 15th
	     ;; color in screen-standard-colors.
	     (* (apply '+ (car (cddr (nth 15 screen-standard-colors)))) 0.6))
