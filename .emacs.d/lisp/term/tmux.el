;;; tmux.el --- define function key sequences and standard colors for tmux
...
;; This is a direct copy of rxvt.el with all instances of the string
;; 'rxvt' replaced by 'tmux'.  If you are used to the colors defined
;; in rxvt.el, this will give you the same colors when running emacs
;; inside tmux provided tmux has its default-terminal option set to
;; 'tmux' or one of its derivatives (e.g. tmux-256color, etc.)

;;; Code:

(defvar tmux-function-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; (define-key tmux-function-map "\e[23~" [f11])
    ;; (define-key tmux-function-map "\e[24~" [f12])
    ...
    map)
  "Function key overrides for tmux.")

(defvar tmux-alternatives-map
  (let ((map (make-sparse-keymap)))
...
(defun terminal-init-tmux ()
  ...
  (let ((map (copy-keymap tmux-alternatives-map)))
  ...
  (let ((m (copy-keymap tmux-function-map)))
    (set-keymap-parent m (keymap-parent input-decode-map))
    (set-keymap-parent input-decode-map m))

  ;; Initialize colors and background mode.
  (tmux-register-default-colors)
  (tmux-set-background-mode)
  ;; This recomputes all the default faces given the colors we've just set up.
  (tty-set-up-initial-frame-faces))

;; Set up colors, for those versions of tmux that support it.
(defvar tmux-standard-colors
  ;; The names of the colors in the comments taken from the tmux.1 man
  ...
  "Names of 16 standard tmux colors, their numbers, and RGB values.")

(defun tmux-rgb-convert-to-16bit (prim)
  "Convert an 8-bit primary color value PRIM to a corresponding 16-bit value."
  (min 65535 (round (* (/ prim 255.0) 65535.0))))

(defun tmux-register-default-colors ()
  "Register the default set of colors for tmux or compatible emulator.

This function registers the number of colors returned by `display-color-cells'
for the currently selected frame."
  (let* ((ncolors (display-color-cells))
	 (colors tmux-standard-colors)
	 (color (car colors)))
    (if (> ncolors 0)
    ...
    (while (and (> ncolors 0) colors)
      (tty-color-define (car color) (cadr color)
			(mapcar 'tmux-rgb-convert-to-16bit
				(car (cddr color))))
	...
	(while (> ncolors 0)
	  (setq color (tmux-rgb-convert-to-16bit (+ 8 (* (- 24 ncolors) 10))))
	  (tty-color-define (format "color-%d" (- 256 ncolors))
			    (- 256 ncolors)
			    (list color color color))
	  (setq ncolors (1- ncolors))))

       ((= ncolors 72) ; tmux-unicode
	;; 64 non-gray colors
	...
	(while (> ncolors 0)
	  (setq color (tmux-rgb-convert-to-16bit
		       (floor
			(if (= ncolors 8)
			    46.36363636
	  ...
	  (tty-color-define (format "color-%d" (- 88 ncolors))
			    ...
			    (list color color color))
	  (setq ncolors (1- ncolors))))
       (t (error "Unsupported number of tmux colors (%d)" (+ 16 ncolors)))))
    ;; Modifying color mappings means realized faces don't use the
    ;; right colors, so clear them.
    (clear-face-cache)))

;; tmux puts the default colors into an environment variable
;; COLORFGBG.  We use this to set the background mode in a more
;; intelligent way than the default guesswork in startup.el.
(defun tmux-set-background-mode ()
  "Set background mode as appropriate for the default tmux colors."
  (let ((fgbg (getenv "COLORFGBG"))
    ...
    (when (and fgbg
      ...
      ;; The next line assumes that tmux-standard-colors are ordered
      ;; by the color index in the ascending order!
      (setq rgb (car (cddr (nth bg tmux-standard-colors))))
      ;; See the commentary in frame-set-background-mode about the
      ;; computation below.
      (if (< (apply '+ rgb)
	     ;; The following line assumes that white is the 15th
	     ;; color in tmux-standard-colors.
	     (* (apply '+ (car (cddr (nth 15 tmux-standard-colors)))) 0.6))
